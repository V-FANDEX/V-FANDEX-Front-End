          ...
          {conditionalOrders.map((order) => {
            ...
            return (
              <div className="stock-row" key={order.id}>
                ...
                <span>{order.direction === 'buyBelow' ? '이하 매수' : '이상 매도'} {currency(order.targetPrice)}</span>
                <strong>{order.quantity.toLocaleString('ko-KR')}주</strong>
                <span className={order.active ? 'pill cyan' : 'pill'}>{order.active ? '활성' : '비활성'}</span>
                <small>{order.executedAt ? `체결 ${dateTime(order.executedAt)}` : `등록 ${dateTime(order.createdAt)}`}</small>
                <button className="icon-button" disabled={!order.active} onClick={() => cancelConditionalOrder(order.id)} aria-label="조건 주문 취소">
                  <X size={18} />
                </button>
              </div>
            );
